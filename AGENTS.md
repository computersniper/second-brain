...
- Course tree root: [[Vault Relationship Map]]
...
- For imported course-image-heavy notes, copy the note's images into a local `assets/` folder beside that note.
- Use simple note-relative Markdown image paths, for example `![image.png](<assets/image.png>)`.
- Shared course images may also live in `Resources/.../Images`, but the note-local `assets/` copy is the most stable for Obsidian rendering.
- Avoid filename-only Obsidian embeds like `![[image.png]]` for imported course-image-heavy notes if Obsidian reports "file not found".
...
- When importing a note with a local asset folder, copy the assets into the note-local `assets/` folder, then rewrite image links to `assets/...`.
...
- SE 笔记位置：`/Notes/Courses/Software Engineering`，核心入口在 [[Vault Relationship Map]] 的 SE branch 下
...
- QF 笔记位置：`/Notes/Courses/Quantum Finance`，核心入口在 [[Vault Relationship Map]] 的 QF branch 下
...
- Tag guide: [[Obsidian Tagging Guide]].
- Obsidian + Codex usage guide: [[Obsidian Codex Usage Guide]].
- Relationship map: [[Vault Relationship Map]].
